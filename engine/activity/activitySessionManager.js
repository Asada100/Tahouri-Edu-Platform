...
const ActivitySessionManager = {
 ...
 load(){const k=this.storageKey();if(!k||typeof SaveManager==="undefined")return null;const s=SaveManager.load(k);if(!s||!s.activityId)return null;this.currentSession=s;return s;},
 ...
 async restoreEngine(a,s){
  ...
  // ClassificationEngine stores its session state directly through getSessionState().
  // Restore that state instead of treating it as a generic/unknown engine state.
  if(n==="classification"||n==="ClassificationEngine"){
      if(typeof e.restoreSession!=="function")return false;
      if(!d || !Array.isArray(d.items) || !Array.isArray(d.categories))return false;
      e.activityData=a;
      if(!e.restoreSession(d))return false;
      if(typeof ClassificationScreen!=="undefined"&&typeof ClassificationScreen.show==="function"){
          ClassificationScreen.currentActivity=a;
          ClassificationScreen.currentState=e.getState();
          ClassificationScreen.lastMessage="";
          ClassificationScreen.show(e.getState());
      }
      return true;
  }
  return false;
