...
const JigsawPuzzle = {
    ...
    start: function (definition) {
        ...
        if (words && words.length >= 2) {
            const pieces = words.map(function (word, index) {
                return { id: `word-${index}`, word: word, correctIndex: index, currentIndex: index };
            });
        ...
        for (let correctIndex = 0; correctIndex < count; correctIndex += 1) {
            pieces.push({ id: `piece-${correctIndex}`, correctIndex: correctIndex, currentIndex: correctIndex });
        }
    ...
    isSolved: function () {
        ...
        if (this.state.mode === "words") {
            return this.state.words.every(function (expectedWord, position) {
                const piece = JigsawPuzzle.getPieceAt(position);
                return !!piece && JigsawPuzzle.normalizeWord(piece.word) === JigsawPuzzle.normalizeWord(expectedWord);
            });
        }
    ...
    getState: function () {
        ...
        return {
            ...
            words: this.state.words ? this.state.words.slice() : undefined,
            pieceCount: this.state.pieceCount,
            difficulty: this.state.difficulty,
            pieces: this.state.pieces.map(function (piece) {
                return { id: piece.id, word: piece.word, correctIndex: piece.correctIndex, currentIndex: piece.currentIndex };
            }),
    ...
    getPieceAt: function (position) {
        if (!this.state) return null;
        return this.state.pieces.find(function (piece) { return piece.currentIndex === position; }) || null;
    },

    isValidPosition: function (position) {
        return Number.isInteger(position) && position >= 0 && this.state && position < this.state.pieceCount;
    },

    normalizeWord: function (word) {
        return String(word == null ? "" : word).replace(/\u200c/g, " ").replace(/\s+/g, " ").trim();
    },
    ...
    shuffleIndexes: function (count) {
        ...
        for (let i = indexes.length - 1; i > 0; i -= 1) {
            ...
            const temp = indexes[i]; indexes[i] = indexes[j]; indexes[j] = temp;
        }
        return indexes;
    }
};
