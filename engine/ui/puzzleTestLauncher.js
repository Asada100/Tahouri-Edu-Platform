...
const PuzzleTestLauncher = {
    ...
    samples: [
        ...
        },
        {
            ...
            title: "جدول متقاطع",
            type: "crossGrid",
            objective: "خانه‌های گمشده را کامل کن",
            instruction: "با توجه به مسیرهای جدول، خانه‌های خالی را کامل کن.",
            difficulty: 2,
            rows: 2,
            cols: 2,
            cells: [2, 4, 6, null],
            ...
            paths: [
                ...
                { cells: [2, 3], operations: ["add"], target: 10 },
                { cells: [1, 3], operations: ["add"], target: 14 }
            ],
            answers: [8]
        }
    ],
    ...
    render: function () {
        ...
        if (!app || document.getElementById("puzzleTestLauncher")) return;

        const buttons = this.samples.map(function (sample, index) {
        ...
        panel.dir = "rtl";
        panel.innerHTML = `
            <div class="puzzleTestLauncherTitle">آزمایش موقت پازل</div>
            <div class="puzzleTestLauncherButtons">${buttons}</div>
        `;
...
console.log("Puzzle Test Launcher v1.0 Ready");
